...
async def test_note_tag_snapshots_and_explicit_clear():
    ...
    with auth_context(user, frozenset(('web_user',))):
        note_id = await NoteService.create(0, 0, 'Original', tags={'hashtags': '#survey'})
        ...
        comments = await NoteCommentQuery.resolve_comments([note], per_note_sort='asc', per_note_limit=None)
...
async def test_note_hashtag_backfill_is_resumable_and_preserves_newer_edits():
    ...
    with auth_context(user, frozenset(('web_user',))):
        ...
        await NoteService.comment(note_id, 'Resolved', 'closed', tags={'hashtags': '#solved'})
        ...
        assert [c['tags'] for c in comments] == [
            {'hashtags': '#survey'}, {'hashtags': '#solved'},
        ]
