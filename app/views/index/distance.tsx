...
const DistanceSidebar = ({
  ...
  const updateGhostPositionNow = (x: number, y: number) => {
    ...
    const firstVisual = getVisualSegment(firstEndIndex)
    let bestEndId = firstSegmentId
    let bestClosestPoint = closestPointOnSegment(
      hitPoint,
      map.project(firstVisual.start),
      map.project(firstVisual.visualEnd),
    )
    let bestDistanceSq =
    ...
    for (let i = 1; i < features.length; i++) {
      ...
      const visual = getVisualSegment(endIndex)
      const closestPoint = closestPointOnSegment(
        hitPoint,
        map.project(visual.start),
        map.project(visual.visualEnd),
      )
      const distanceSq =
